...
def pdf_gg_distributions_plot(
    dates: List[str], time_step: str, N_values: List[int]
) -> None:
    ...
    for N_value in N_values:
        gg_distribution_log: np.ndarray = (
            exact_distributions_tools.pdf_gaussian_gaussian(x_val_log, N_value, 1)
        )
        ax1.semilogy(x_val_log, gg_distribution_log, "-", lw=5, label=f"N = {N_value}")
        ax2.loglog(x_val_log, gg_distribution_log, "-", lw=5, label=f"N = {N_value}")
...
def pdf_ga_distributions_plot(
    ...
    for N_value in N_values:
        for L_value in L_values:
            ga_distribution_log: np.ndarray = (
                exact_distributions_tools.pdf_gaussian_algebraic(
                    x_val_log, K_value, L_value, N_value, 1
                )
            )
            ax1.semilogy(
                x_val_log,
...
def pdf_ag_distributions_plot(
    ...
    for N_value in N_values:
        for l_value in l_values:
            ag_distribution_log: np.ndarray = (
                exact_distributions_tools.pdf_algebraic_gaussian(
                    x_val_log, K_value, l_value, N_value, 1
                )
            )
            ax1.semilogy(
...
def main() -> None:
    ...
    pdf_ga_distributions_plot(["1990-01-01", "2020-12-31"], "1d", [2, 3, 4], 244, [150])
    pdf_ag_distributions_plot(
        ["1990-01-01", "2020-12-31"], "1d", [2, 3, 4, 5], 244, [150]
    )
    pdf_aa_distributions_plot(
        ["1990-01-01", "2020-12-31"], "1d", [2, 3, 4, 5], 244, [150], [150]
    )
