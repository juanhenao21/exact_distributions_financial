...
The module contains the following functions:
    ...
    * epochs_sim_agg_ret_pairs - plots the simulated aggregated rotated and scaled returns
...
def epochs_sim_agg_ret_pairs(
    K_value: int, normalized: bool = False, kind: str = "gaussian"
) -> None:
    """Plots the simulated aggregated rotated and scaled returns without
       normalization.
    ...
    for epochs_len, marker in zip([10, 25, 40, 55, 100], markers):
        agg_ret_pairs = epochs_sim_analysis.epochs_sim_agg_returns_market_data(
            0.3, 2, K_value, 200, epochs_len, normalized, kind
        )

        agg_ret_pairs = agg_ret_pairs.rename(f"Epochs T={epochs_len}")

        # Log plot
        plot = agg_ret_pairs.plot(
    ...
    if kind == "gaussian":
        gaussian: np.ndarray = epochs_sim_tools.gaussian_distribution(0, 1, x_values)
        plt.semilogy(x_values, gaussian, "-", lw=10, label="Gaussian")
    if kind == "algebraic":
        algebraic: np.ndarray = epochs_sim_tools.algebraic_distribution(
            K_value, (10 + K_value) / 2, x_values
        )
        plt.semilogy(x_values, algebraic, "-", lw=10, label="Algebraic")

    plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.13), ncol=3, fontsize=30)
    ...
    if not normalized and (kind == "gaussian"):
        figure.savefig(f"../plot/06_epochs_sim_gauss_agg_ret_pairs_no_norm.png")
    if not normalized and (kind == "algebraic"):
        figure.savefig(f"../plot/06_epochs_sim_alg_agg_ret_pairs_no_norm.png")
...
def epochs_sim_ts_norm_agg_ret(K_value: int, kind: str) -> None:
...
def epochs_var_win_all_empirical_dist_returns_market_plot(
    ...
    epochs_sim_tools.function_header_print_plot(function_name, ["", ""], "", "", "")
    ...
    try:
        ...
        for idx, date_val in enumerate(dates):
            ...
            gaussian: np.ndarray = epochs_sim_tools.gaussian_distribution(
                0, 1, x_values
            )
            ...
            ax3.legend(
                loc="upper center", bbox_to_anchor=(1.0, -0.2), ncol=6, fontsize=15
            )
