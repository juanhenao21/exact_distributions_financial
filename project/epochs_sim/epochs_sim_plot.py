...
def epochs_sim_agg_returns_market_plot(
    agg_ret: pd.Series, epochs_len: int, K_value: int, kind: int = "gaussian"
) -> None:
    ...
    epochs_sim_tools.function_header_print_plot(
    ...
    plt.xlabel(f"Aggregated simulated returns - epochs {epochs_len}", fontsize=25)
    ...
    # Plotting
    epochs_sim_tools.save_plot(
        figure_log, function_name + "_" + kind, [""], "", str(epochs_len), "", sim=True
    )
...
def epochs_sim_agg_returns_cov_market_plot(agg_ret: pd.Series, epochs_len: int) -> None:
    """Plots the aggregated distribution of simulated returns for a market.
    ...
    # Plotting
    epochs_sim_tools.save_plot(
        figure_log, function_name, [""], "", str(epochs_len), "", sim=True
    )
...
def epochs_aggregated_dist_returns_market_plot(
    dates: List[str], time_step: str, window: str, K_value: str
) -> None:
